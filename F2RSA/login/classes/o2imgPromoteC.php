<?php
		...
		//This class creates the directory for the images location and stores the image.	
			function o2imgPromoteC(){
		if(isset($_POST['o2imgPromote'])){
			$ptitle = @$_GET['ptitle'];
...
$uploadfile = $madeDir . basename($_FILES['o2img']['name']);
}else{
...
/*if (file_exists($uploadfile)) {
	...
	//echo getcwd();
move_uploaded_file($_FILES['o2img']['tmp_name'], $uploadfile);
...
$id = 1;
$sql4InsertPicture = "UPDATE promotedrigs SET o2img = '".$uploadfile."' WHERE username = '$username' AND id = '$id'";
	$insert_o2img = mysql_query($sql4InsertPicture) or die("Could not Insert Picture");	
echo '<b>Picture Named: '.basename($uploadfile).' was uploaded Successfully</b>';
//end else}
}//end if isset o2imgPromote
echo '<br />';
